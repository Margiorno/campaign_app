            ...
            {products.map(product => (
                <div key={product.id} style={styles.card}>
                    ...
                    <div style={styles.cardActions}>
                        <button onClick={() => handleEdit(product)} style={styles.editButton}>Edytuj</button>
                        <button onClick={() => handleDelete(product.id)} style={styles.deleteButton}>Usuń</button>
                    </div>
                </div>
            ))}
            {isModalOpen && (
                <Modal onClose={() => setIsModalOpen(false)}>
                    ...
                    <form onSubmit={handleUpdate}>
                        ...
                        />
                        <button type="submit" style={styles.submitButton}>Zapisz zmiany</button>
                    </form>
                </Modal>
            )}
        </div>
...
const styles = {
    ...
    card: { border: '1px solid #e0e0e0', borderRadius: '8px', padding: '15px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column' },
    cardTitle: { margin: '0 0 10px 0', fontSize: '1.1rem' },
    cardDescription: { flexGrow: 1, color: '#666', fontSize: '0.9rem' },
